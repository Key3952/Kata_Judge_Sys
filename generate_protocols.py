...
def _write_judge_pair_sheet(wb: Workbook, sheet_name: str, comp_name: str, discipline: str, judge_name: str, pair_name: str, techniques: List[str], details: List[dict]):
    ...
    ws["A5"] = f"Судья: {judge_name}"
    ws["A6"] = f"Пара: {pair_name}"
    ...
    ws["I9"] = "-10"
    for col in "DEFGHI":
        ws[f"{col}9"].font = Font(italic=True)
...
def generate_competition_protocols(
    ...
    try:
        ...
        for dk in dlist:
            ...
            generated["disciplines"][dk] = [files["xlsx"], files["pdf"]]
